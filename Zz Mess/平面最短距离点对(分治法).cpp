...
bool sortBy_x(const point &point1, const point &point2) //根据x坐标从小到大排序
{
    if (point1.x == point2.x)
        return point1.y < point2.y;
    return point1.x < point2.x;
}
bool sortBy_y(const point &point1, const point &point2)    //根据纵坐标从小到大排序
{
    return point1.y < point2.y;
}
...
double getMin(vector<point>& vec, int low, int high)
{
    ...
    else
    {
        ...
        {
            if(i+7 < res.size()){            //求距离的点 为与i纵坐标的距离在d以内 (进行优化,最多比较相邻的6个点)
                for (j = i + 1; j < i + 7; j++)
                {
            ...
            else{                                   //求距离的点 为与i纵坐标的距离在d以内 (进行优化,最多比较相邻的6个点)
